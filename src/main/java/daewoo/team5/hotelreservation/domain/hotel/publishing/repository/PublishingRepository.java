...
public interface PublishingRepository extends JpaRepository<Publishing, Long> {
    ...
    sql 문 사용
     */
    @Query("select p.hotelName as name,a.city as sido from publishing p join Room a on p.publishingId = a.publishing.publishingId")
    public List<TestProjection> getPublishings();   //리스트 형식으로 패키지(projection)의 프로젝션인 인터페이스 1개 생성 후
